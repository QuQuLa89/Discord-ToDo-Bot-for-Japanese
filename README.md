...
- OS: Windows 10 / 11
- Python: 3.10 系（`pyproject.toml` の `requires-python` に準拠）
...
1. Python 3.10と[Poetry](https://python-poetry.org/docs/#installation)をインストールします。
2. このフォルダで依存パッケージをインストールします（初回は仮想環境も自動作成されます）。
...
開発環境で動作確認したあと、次のコマンドで単体exeを作成できます（`pyinstaller` は dev 依存として `pyproject.toml` に含まれています）。

```powershell
poetry run pyinstaller --onefile --name todo-bot main.py
```
